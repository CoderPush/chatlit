...
def render_body(st):
    with st.expander(get_expander_text(st)):
        ...
        if user_info:
            ...
            st.write("While it's useful to resume past conversations, sometimes you may want to clear your chat history.")
            placeholder = st.empty()
            with placeholder:
                clear_history = st.button("Clear History", key="button_clear_history", type='primary')
            if clear_history:
